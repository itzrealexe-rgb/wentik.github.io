            ...
            html += `
                <div style="margin-top:40px;padding:20px;background:rgba(235,69,158,0.1);border:1px solid rgba(235,69,158,0.2);border-radius:16px;">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;">
                        <button onclick="adminAction('servers')" class="btn btn-primary" style="font-size:12px;">📡 Servers</button>
                        <button onclick="adminAction('users')" class="btn btn-primary" style="font-size:12px;">👥 Users</button>
                        <button onclick="adminAction('stats')" class="btn btn-primary" style="font-size:12px;">📊 Refresh</button>
...
async function adminAction(action) {
    ...
    try {
        if (action === 'servers') {
            ...
            const res = await fetch(`${API_BASE}/api/stats`);
            const data = await res.json();
            output.innerHTML = `📊 Updated: ${data.stats?.servers || 0} servers, ${data.stats?.users || 0} users`;
